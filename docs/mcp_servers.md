...
| Secrets | `scripts/mcp_secrets.mjs` | `SECRETS_ALLOWLIST` | Allowlisted environment variable access. |
...
| `MCP_SSE_HMAC_SECRET`  | Adds HMAC signature line per ingested event           | (disabled)         |
...
When `MCP_SSE_HMAC_SECRET` is provided, the gateway computes an HMAC (SHA-256) over the raw
...
`MCP_SSE_HMAC_SECRET` omits the line (backwards compatible).
