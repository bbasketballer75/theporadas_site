  ...
  constructor() {
    ...
    if (config.mcpAuthMode === "jwt") {
      if (environment === "production" && !config.mcpAuthSecretKey) {
        logger.fatal(
          "CRITICAL: MCP_AUTH_REDACTED_BY_AUDIT_ISSUE_70_KEY is not set in production for JWT auth.",
          context,
        );
        throw new McpError(
          BaseErrorCode.CONFIGURATION_ERROR,
          "MCP_AUTH_REDACTED_BY_AUDIT_ISSUE_70_KEY must be set for JWT auth in production.",
          context,
        );
      } else if (!config.mcpAuthSecretKey) {
        logger.warning(
          "MCP_AUTH_REDACTED_BY_AUDIT_ISSUE_70_KEY is not set. JWT auth will be bypassed (DEV ONLY).",
          context,
        );
        this.secretKey = null;
      } else {
