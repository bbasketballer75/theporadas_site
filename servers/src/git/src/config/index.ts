...
const EnvSchema = z.object({
  ...
  MCP_AUTH_REDACTED_BY_AUDIT_ISSUE_70_KEY: z
    .string()
    .min(
      32,
      "MCP_AUTH_REDACTED_BY_AUDIT_ISSUE_70_KEY must be at least 32 characters long for security reasons.",
    )
    .optional(),
...
export const config = {
  ...
  mcpAllowedOrigins: env.MCP_ALLOWED_ORIGINS?.split(",")
    ...
    .filter(Boolean),
  /** Auth secret key (JWTs, http transport). From `MCP_AUTH_REDACTED_BY_AUDIT_ISSUE_70_KEY`. CRITICAL. */
  mcpAuthSecretKey: env.MCP_AUTH_REDACTED_BY_AUDIT_ISSUE_70_KEY,
