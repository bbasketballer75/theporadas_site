...
describe('buildTree exclude patterns', () => {
    ...
    beforeEach(async () => {
        ...
        // Create test files
        await fs.writeFile(path.join(testDir, '.env'), 'REDACTED_BY_AUDIT_ISSUE_70=value');
        await fs.writeFile(path.join(testDir, '.env.local'), 'LOCAL_REDACTED_BY_AUDIT_ISSUE_70=value');
