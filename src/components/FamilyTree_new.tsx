...
export function FamilyTree({ width = 800, height = 600, onMemberClick }: FamilyTreeProps) {
  ...
  const [loading, setLoading] = useState(true);`r`n  const [searchTerm, setSearchTerm] = useState('');`r`n  const [filteredMembers, setFilteredMembers] = useState<FamilyMember[]>([]);
  const [error, setError] = useState<string | null>(null);

  // Browser detection for D3.js compatibility fixes`r`n`r`n  // Filter members based on search term`r`n  const filterMembers = useCallback((members: FamilyMember[], term: string) => {`r`n    if (!term.trim()) return members;`r`n`r`n    const lowerTerm = term.toLowerCase();`r`n    return members.filter(member =>`r`n      member.name.toLowerCase().includes(lowerTerm) ||`r`n      member.relationship.toLowerCase().includes(lowerTerm) ||`r`n      (member.description && member.description.toLowerCase().includes(lowerTerm)) ||`r`n      (member.birthDate && member.birthDate.toLowerCase().includes(lowerTerm))`r`n    );`r`n  }, []);`r`n`r`n  // Update filtered members when search term or family members change`r`n  useEffect(() => {`r`n    setFilteredMembers(filterMembers(familyMembers, searchTerm));`r`n  }, [familyMembers, searchTerm, filterMembers]);
